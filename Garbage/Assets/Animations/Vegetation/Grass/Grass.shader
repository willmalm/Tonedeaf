Shader "Anarchy/Grass" {
	Properties {
		_Color ("Color A Base", Color) 	= (0,0.1,0,1)
		_ColorT ("Color A Top", Color) 	= (1,1,1,1)
		_ColorB ("Color B Base", Color) = (0,0.1,0,1)
		_ColorBT ("Color B Top", Color) = (1,1,1,1)
	...
	SubShader {
         ...
         void disp (inout appdata v)
            {
               ...
               float3 worldPos = mul(_Object2World,v.vertex.xyz);
					...
					float hardness = tex2Dlod(_MainTex, float4(v.texcoord.xy,0,0)).b;
					hardness = 1-hardness;
		...
		fixed4 _Color,_ColorT,_ColorB,_ColorBT;
		...
		void surf (Input IN, inout SurfaceOutputStandard o) {
			...
			c.rgb = lerp(	lerp(_Color,_ColorT,topBot),	lerp(_ColorB,_ColorBT,topBot),	ab);
